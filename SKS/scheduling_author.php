...
$sql="SELECT * FROM rad_2 WHERE rad_author='$rt_author' ORDER BY rad_time_broadcasr, rad_date_broadcast ASC";
...
$weekday2=array("星期日","星期一","星期二","星期三","星期四","星期五","星期六");
 ...
 foreach ($arr as &$record1) { 
   	if($record1['weekday']==$weekday & $record1['rad_time_broadcasr']==$time){
		//echo $record1['weekday']."--".$record1['rad_time_broadcasr'];
 
 ?>		
 			<td bgcolor="#F1F1F1"><?php echo $record1['rad_date_broadcast']."<br>".$record1['rad_title']."<br>".$record1['rad_time_broadcasr']?></td>
 
   	<?php
    }else{
		//echo $record1['weekday']."--".$record1['rad_time_broadcasr'];
		$weekday=$record1['weekday'];
		$time=$record1['rad_time_broadcasr'];
		?>
        </tr>
        <tr>
    		<td bgcolor="#B1B1B1"><?php echo $weekday2[$record1['weekday']] ?></td>
			<td bgcolor="#999999"><?php echo $record1['rad_time_broadcasr'] ?></td>
        <td bgcolor="#F1F1F1"><?php echo $record1['rad_date_broadcast']."<br>".$record1['rad_title']."<br>".$record1['rad_time_broadcasr']			?>
       </td>
		<?php
	}
