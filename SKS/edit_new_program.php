...
if ( $newSN > 336 ) {
  ...
  $sql3 = "SELECT * FROM rad_timetable WHERE rt_sn='$newSN' AND rt_author='$rpAuthor'"; //搜索資料指令
...
?>
<form id="form1" name="form1" method="post" action="edit_new_program2.php">
  <table width="866" border="0" cellspacing="4" cellpadding="4">
    <tr>
      <td colspan="4" align="center" bgcolor="#999999">時段修改</td>
    </tr>
    <tr>
      <td width="136" bgcolor="#CCCCCC">rt_grid</td>
      <td width="233"><?php echo $arr[0]['rt_grid']?></td>
      <td width="139" bgcolor="#CCCCCC">主持人rt_author</td>
      <td width="306"><?php echo $_SESSION['m_name']?></td>
    </tr>
    <tr>
      <td bgcolor="#CCCCCC">廣播時段(半小時)</td>
      <td><?php echo $arr[0]['rt_time']?> 到<br>
        <?php echo $time1 ?></td>
      <?php
      if ( $next == '1' ) {
        if ( $arr3[ 0 ][ 'rt_author' ] == $rpAuthor & $arr3[ 0 ][ 'rt_relate' ] == $sn ) {
          ?>
      <td bgcolor="#CCCCCC">增加時段(半小時)</td>
      <td><input type="checkbox" name="checkbox" id="checkbox" checked="checked" disabled/>
        到<br>
        <?php echo $time2 ?></td>
      <?php }else{?>
    ...
    <tr>
      ...
      <td><?php echo $day?>
        <input type="hidden" name="day" id="day" value="<?php echo $arr[0]['rt_day']?>" disabled /></td>
      <td bgcolor="#CCCCCC">節目主題rt_prog</td>
      <td><select name="select" id="select" selected="<?php echo $arr[0]['rt_prog']?>">
          <?php
          for ( $ii = 0; $ii < $number_result2; $ii++ ) {
            ?>
    ...
    <tr>
      ...
      <td></td>
      <td>&nbsp;</td>
      <td><input type="submit" name="button" id="button" value="確定修改" /></td>
    </tr>
  </table>
</form>
</body>
